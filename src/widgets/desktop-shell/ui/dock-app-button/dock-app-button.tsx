...
type DockAppButtonProps = {
  item: DockAppState;
  onActivate: () => void;
  onOpenMenu: (anchor: WindowPosition) => void;
};

export function DockAppButton({ item, onActivate, onOpenMenu }: DockAppButtonProps) {
  ...
  return (
    <button
      ...
      className="group relative flex cursor-pointer flex-col items-center gap-2 rounded-2xl px-1 pb-1 pt-0 text-white transition duration-200 hover:-translate-y-2 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white"
      aria-label={label}
    >
      ...
      <span className="flex items-center gap-1.5">
        <span
          className={cn(
            ...
            item.visibleCount > 0 ? "w-4 opacity-100" : "w-1.5 opacity-0",
          )}
        />
        <span
          className={cn(
